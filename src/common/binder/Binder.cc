...
//!VH record the current UE's processing demand
void Binder::setUECurrentProcessingDemand(MacNodeId id, double procDemand)
{
        //std::vector<UeInfo*>::iterator it = std::find_if(ueList_.begin (), ueList_.end (), boost::bind(&UeInfo::id, _1) == id );
        //auto iter = std::find_if(ueList_.begin (), ueList_.end (), boost::bind(&UeInfo::id, _1) == id );
        std::vector<UeInfo*>::iterator it = std::find_if(ueList_.begin (), ueList_.end (), boost::bind(&UeInfo::id, _1) == id );
        if ( it != ueList_.end() )
        {
           UeInfo* info = *it;
           //ueList_.end().lastProcDemand = procDemand;
           info->lastProcDemand = procDemand;
        }
}

bool search_UE(UeInfo *ue, MacNodeId nodeId)
{
    //MacNodeId id = 1;
    if (ue->id == nodeId)
        return true;
...
//!VH calculate and record the current CN's processing demand
double Binder::computeAndRecordProcessingDemand(LteAllocationModule *allocator, MacNodeId nodeId)
{
    ...
    for ( ; it != et; ++it)
    {
        ...
        if (info->id == nodeId)
        {
            ...
            // find all UEs under this BS
            std::vector<UeInfo*>::iterator itUE = find_if(ueList_.begin(), ueList_.end(), [&nodeId](UeInfo *ue){
                return ue->cellId == nodeId;
            });

            //std::vector<UeInfo*>::iterator itUE = std::find_if(ueList_.begin (), ueList_.end (), [](UeInfo ueList_){return ueList_.cellId == id;});
            //for ( ; itUE != ueList_.end(); ++itUE)
            while(itUE != ueList_.end() )
            {
                UeInfo* infoUE = *itUE;
                if (infoUE->cellId == nodeId)
                {
...
double Binder::computeBSProcessingDemand(LteAllocationModule *allocator)
{
    MacNodeId nodeId = 1;
    std::vector<EnbInfo*>::iterator it = enbList_.begin(), et = enbList_.end();
    for ( ; it != et; ++it)
    {
        ...
        if (info->id == nodeId)
        {
            double BSProcDemand = 0;
            // find all UEs under this BS
            //std::vector<UeInfo*>::iterator itUE = std::find_if(ueList_.begin (), ueList_.end (), boost::bind(&UeInfo::cellId, _1) == nodeId );
            //std::vector<UeInfo*>::iterator itUE = std::find_if(ueList_.begin(), ueList_.end(), boost::bind(&UeInfo::cellId, _1) == nodeId );
            std::vector<UeInfo*>::iterator itUE = find_if(ueList_.begin(), ueList_.end(), [&nodeId](UeInfo *ue){
                return ue->cellId == nodeId;
            });
    ...
    for (const auto& outer_pair : placementSolution) {
        ...
        if (outer_key == nodeId) {
            ...
            for (const auto& inner_pair : inner_map) {
                ...
                double inner_val = inner_pair.second;

                //searching the BS computing load
                for (auto enb : enbList_) {
                    // Check if the id field matches the target id
                    if (enb->id == inner_key) {
                        BSProcDemand += enb->lastBSProcDemand * inner_val;
                        break; // Exit the loop after finding the first matching entry
                    }
                }

                // Do something with the key-value pair
                //std::cout << "outer key: " << outer_key << ", inner key: " << inner_key << ", inner value: " << inner_val << std::endl;
            }
        }
    }
    return BSProcDemand;
