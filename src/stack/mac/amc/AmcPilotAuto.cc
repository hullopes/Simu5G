...
const UserTxParams& AmcPilotAuto::computeTxParams(MacNodeId id, const Direction dir, double carrierFrequency)
{
    ...
    //VH _txMode
    //int txModeId = binder_->getCurrentTxModeId();
    ...
    //VH _cqi _RI
    if (binder_->getCurrentTxModeId() == 3) //see TxMode (src/common/LteCommonEnum_m.h), TxMode 3 = OL_SM
        info.writeCqi(std::vector<Cqi>(sfb.getRi(), chosenCqi)); //!VH use only for SM txMode
    else
        info.writeCqi(std::vector<Cqi>(1, chosenCqi)); //!VH use only for TD
