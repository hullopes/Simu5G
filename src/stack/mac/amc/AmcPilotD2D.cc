...
//    preconfiguredTxParams_->setD2DEnabled(true);
    //VH _txMode
    preconfiguredTxParams_->writeTxMode(TxMode(binder_->getCurrentTxModeId()));
    ...
    //VH _cqi _RI
    if (binder_->getCurrentTxModeId() == 3)
        preconfiguredTxParams_->writeCqi(std::vector<Cqi>(ri,cqi));//!VH use only for SM txMode
    else
        preconfiguredTxParams_->writeCqi(std::vector<Cqi>(1,cqi)); //!VH for TD
...
const UserTxParams& AmcPilotD2D::computeTxParams(MacNodeId id, const Direction dir, double carrierFrequency)
{
    ...
    //VH _txMode
    TxMode txMode = TxMode(binder_->getCurrentTxModeId());
    //TxMode txMode = OL_SPATIAL_MULTIPLEXING;
...
//        b.insert(cellWiseBand);
    ...
    //VH _cqi _RI
    if (binder_->getCurrentTxModeId() == 3)
        info.writeCqi(std::vector<Cqi>(sfb.getRi(),chosenCqi));//!VH use only for SM txMode
    else
        info.writeCqi(std::vector<Cqi>(1,chosenCqi)); //!VH for TD
