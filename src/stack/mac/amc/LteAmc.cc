...
unsigned int LteAmc::computeBitsOnNRbs_MB(MacNodeId id, Band b,  unsigned int blocks, const Direction dir, double carrierFrequency)
{
    ...
    //VH _txMode
    const unsigned int* tbsVect = itbs2tbs(mod, TxMode(getBinder()->getCurrentTxModeId()), 1, iTbs - i); //!VH TM1 (1 layer, 1 antenna)
    if (getBinder()->getCurrentTxModeId() == 3)
        tbsVect = itbs2tbs(mod, TxMode(getBinder()->getCurrentTxModeId()), layers[0], iTbs - i); //!VH TM1 (1 layer, 1 antenna)
    //const unsigned int* tbsVect = itbs2tbs(mod, TRANSMIT_DIVERSITY, 1, iTbs - i); //!VH TM1 (1 layer, 1 antenna)
    //const unsigned int* tbsVect = itbs2tbs(mod, TRANSMIT_DIVERSITY, layers[0], iTbs - i); //!VH TM2 (2 layer, 2 antenna)
    //const unsigned int* tbsVect = itbs2tbs(mod, SINGLE_ANTENNA_PORT0, layers[0], iTbs - i);
    //const unsigned int* tbsVect = itbs2tbs(mod, OL_SPATIAL_MULTIPLEXING, 2, iTbs - i);
...
unsigned int LteAmc::computeBitsPerRbBackground(Cqi cqi, const Direction dir, double carrierFrequency)
{
    ...
    //const unsigned int* tbsVect = itbs2tbs(mod, OL_SPATIAL_MULTIPLEXING, layers, iTbs - i);
