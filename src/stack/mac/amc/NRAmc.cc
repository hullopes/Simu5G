...
std::tuple<int, double> NRAmc::computeCodewordTbs(UserTxParams* info, Codeword cw, Direction dir, unsigned int numRe)
{
    ...
    if (cw < info->readCqiVector().size()){
        ...
        mcsElem = getMcsElemPerCqi(info->readCqiVector().at(0), dir);
    }
    ...
    // alpha*((3*a+(a*a)+(o*r*l)/3)*(gamma/5))
    double a = info->readRank();
    //double cnProcessingDemand = 1*((3*a+pow(a,2)+(modFactor*coderate*l)/3)*(25/5));
    double cnProcessingDemand = (3*a+pow(a,2)+(modFactor*coderate*l)/3);


    return {computeTbsFromNinfo(floor(nInfo),coderate), cnProcessingDemand};
}
...
unsigned int NRAmc::computeBitsOnNRbs(MacNodeId id, Band b, unsigned int blocks, const Direction dir, double carrierFrequency)
{
    ...
    //!VH changing the tbs processing cost
    binder_->setUECurrentProcessingDemand(id, tbsCostFinal);
