...
UserTxParams* LteMacUeD2D::getPreconfiguredTxParams(MacNodeId id)
{
    ...
    txParams->isSet() = true;
    //VH _txMode
    txParams->writeTxMode(TxMode(getBinder()->getCurrentTxModeId()));
    ...
    if (getBinder()->getCurrentTxModeId() == 3)
        txParams->writeCqi(std::vector<Cqi>(ri,cqi));//!VH use only for SM txMode
    else
        txParams->writeCqi(std::vector<Cqi>(1,cqi)); // only for TD
