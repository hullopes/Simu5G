...
unsigned int LteSchedulerEnb::scheduleGrant(MacCid cid, unsigned int bytes, bool& terminate, bool& active, bool& eligible, double carrierFrequency, BandLimitVector* bandLim, Remote antenna, bool limitBl)
{
    ...
    if (getBinder()->getCurrentTxModeId() != 3)
        numCodewords = 1; //VH use only for Single antenna / TD txMode
    //!VH commented the numCodewords def above
...
//!VH computing CN processing demand
void LteSchedulerEnb::CNStatistics(MacNodeId nodeId)
{
    //computing for each UE under this BS
    double BsCost = getBinder()->computeAndRecordProcessingDemand(allocator_, nodeId);
        //MacCellId cellId = getCellInfo()->getMacCellId();
    //if (cost > 0.0)
